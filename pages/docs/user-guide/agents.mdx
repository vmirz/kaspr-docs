...
For graceful shutdown logic, check `app.should_stop` inside long-running or blocking work and exit cleanly when it becomes `True`.
...
def process(value):
    ...
    for chunk in chunks:
        # If a shutdown signal arrives mid-work, stop early and return.
        if app.should_stop:
            flush_partial_results(processed)
            value["interrupted"] = True
            value["processed_chunks"] = len(processed)
            return value

        processed.append(do_expensive_io(chunk))

    finalize_results(processed)
    return value
```
