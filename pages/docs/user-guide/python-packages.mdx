...
When `pip install` needs SSH-specific configuration such as a deploy key, known hosts file, or `GIT_SSH_COMMAND`, you can customize only the package installer init container via `template.pythonPackagesInitContainer`.

More generally, this volume-mount pattern is not limited to SSH. It is generic enough to support other authentication methods as well, such as mounted certificates, custom credential files, or other tool-specific auth material that `pip`, `git`, or related helpers need during package installation.

```yaml
pythonPackages:
  packages:
    - git+ssh://git@github.com/my-org/private-payments-lib.git@v1.2.0#egg=private-payments-lib
template:
  pod:
    volumes:
      - name: github-ssh
...
- The Secret should contain both the private key and a `known_hosts` file.
